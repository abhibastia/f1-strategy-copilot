...
| Tests | 84, all passing — `pytest tests/ -q`; 29 run with no database |
| Jobs and pipelines | `databricks.yml` + `resources/f1_jobs.yml` — deploy with `databricks bundle deploy` |

52 commits, 77 files, ~8,300 lines of Python.
...
The full repository is on GitHub, linked above. This archive carries the source,
the documentation and the screenshots; four things are left out because they are
regenerable or duplicated, and the archive is read rather than run:

| Left out | Why |
|---|---|
| `data/` (except `demo_transcripts.json`) | Harvested API output, regenerated by `harvest/`. It was two thirds of the repository's text. The transcripts stay because `RESULTS.md` is derived from them. |
| `app/f1lake/`, `mcp_server/f1lake/`, `app/f1_broker.py` | Copies made at build time by `scripts/build_app.sh`, because a Databricks App deploys from a single source path. The originals are in `f1lake/` and `mcp_server/`. |
| `docs/diagrams/` | The same two PNGs ship as `screenshots/18` and `19`. |

Deploying from this archive therefore needs one command first —
`./scripts/build_app.sh app && ./scripts/build_app.sh mcp_server` — or a clone
of the repository, where nothing is missing.
